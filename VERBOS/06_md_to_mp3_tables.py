...
import time  # Aunque ya no usemos time.sleep directamente para pausas SSML
...
def envolver_en_voz(texto, idioma_actual, voz_esp, voz_eng):
    """Envuelve el texto en las etiquetas <voice> según el idioma detectado."""
    ...
    for i, palabra in enumerate(palabras):
        ...
        idioma_palabra = detectar_idioma(palabra, siguiente_palabra)
...
def procesar_tabla(linea, separador=", "):
    """Procesa una línea de tabla Markdown, insertando un separador entre columnas."""
    columnas = [col.strip() for col in linea.strip('|').split('|')]
    texto_con_separador = separador.join(columnas)
    return texto_con_separador

def main():
    # Voces
    voz_esp = "Microsoft Helena Desktop"    # Español
    voz_eng = "Microsoft Zira Desktop"     # Inglés
    separador_columna = ", " # Usaremos una coma como separador
    ...
    for archivo in archivos_md:
        try:
            ...
            # Leer y procesar el contenido línea por línea
            try:
                with open(ruta_md, "r", encoding="utf-8") as f:
                    for linea in f:
                        if linea.strip().startswith('|'):
                            if re.match(r'\|[-]+\|[-]+\|', linea.strip()): # Detectar la línea de separación
                                es_separador = True
                                continue # Omitir la línea de separadores
                            else:
                                contenido_procesado.append(procesar_tabla(linea, separador_columna))
                                en_tabla = True
                        else:
                            contenido_procesado.append(limpiar_markdown(linea))
                            en_tabla = False
                            es_separador = False
            ...
            texto_con_voz = envolver_en_voz("\n".join(contenido_procesado), 'es', voz_esp, voz_eng)
